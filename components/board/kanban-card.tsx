    ...
    >
      ...
      {(hasDescription || hasDue || members.length > 0) && (
        <div className="mt-1.5 flex items-center gap-2">
          ...
          {/* Avatares dos membros a direita (max 5 visiveis + overflow) */}
          {members.length > 0 && (
            <div className="flex -space-x-1 shrink-0" title={members.map((m) => m.name).join(", ")}>
              {members.slice(0, 5).map((member) => {
                ...
                return (
                  <div
                    key={member.id}
                    title={member.name}
                    className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold text-white ring-2 ring-white overflow-hidden ${
